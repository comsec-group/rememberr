...
def plot_timeline():
    ...
    for cpu_id, cpu_name in enumerate(intel_cpu_names):
        axs.plot(Xs_intel[cpu_id], Ys_intel[cpu_id], marker='.', label=intel_cpu_prettynames[cpu_name])
    # Set the X labels
    xtick_locations = []
    for year in range(2008, 2027):
        xtick_locations.append(tupldate_to_int(1, year))
    axs.set_xticks(xtick_locations, range(2008, 2027))
    axs.set_xlim(-2, xtick_locations[-1])
    axs.grid()
    axs.legend(loc="lower left", ncol=8)
    ...
    plt.savefig(os.path.join(os.environ['ERRATA_BUILDDIR'], 'figures', "timeline_intel.pdf"), dpi=300)
    plt.savefig(os.path.join(os.environ['ERRATA_BUILDDIR'], 'figures', "timeline_intel.png"), dpi=300)
