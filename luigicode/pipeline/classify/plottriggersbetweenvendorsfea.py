...
triggers_FEA = list(filter(lambda s: '_FEA_' in s, trigger_classes_map))

#####
# Luigi task
#####

class PlotTriggerBetweenVendorsFEA(luigi.Task):
    def __init__(self, *args, **kwargs):
        super(PlotTriggerBetweenVendorsFEA, self).__init__(*args, **kwargs)
    ...
    def run(self):
        ...
        for trigger, erratumdict in intel_trigger_categorization_bool.items():
            ...
            if len(trigger) <= 8 or trigger[:8] != 'trg_FEA_':
                continue
        ...
        for trigger_class_id in range(len(triggers_FEA)):
        ...
        plt.savefig(os.path.join(target_dir, "triggersbetweenvendorsfea.pdf"), dpi=300)
        plt.savefig(os.path.join(target_dir, "triggersbetweenvendorsfea.png"), dpi=300)
