...
triggers_EXT = list(filter(lambda s: '_EXT_' in s, trigger_classes_map))

#####
# Luigi task
#####

class PlotTriggerBetweenVendorsEXT(luigi.Task):
    def __init__(self, *args, **kwargs):
        super(PlotTriggerBetweenVendorsEXT, self).__init__(*args, **kwargs)
    ...
    def run(self):
        ...
        for trigger, erratumdict in intel_trigger_categorization_bool.items():
            # Only look at FLT triggers
            if len(trigger) <= 8 or trigger[:8] != 'trg_EXT_':
                continue
            for ischosen in erratumdict.values():
                num_trgs_per_class_intel[trigger] += int(ischosen)
        # AMD
        for trigger, erratumdict in amd_trigger_categorization_bool.items():
            ...
            if len(trigger) <= 8 or trigger[:8] != 'trg_EXT_':
                continue
            for ischosen in erratumdict.values():
                num_trgs_per_class_amd[trigger] += int(ischosen)

        ########################################
        # Compute the total number of triggers
        ########################################

        tot_num_trgs_intel = 0
        tot_num_trgs_amd = 0

        for errdict in intel_trigger_categorization_bool.values():
            for ischosen in errdict.values():
                tot_num_trgs_intel += int(ischosen)
        for errdict in amd_trigger_categorization_bool.values():
            for ischosen in errdict.values():
                tot_num_trgs_amd += int(ischosen)
        ...
        for trg in triggers_EXT:
            Ys_intel.append(100*num_trgs_per_class_intel[trg]/tot_num_trgs_intel)
            Ys_amd.append(100*num_trgs_per_class_amd[trg]/tot_num_trgs_amd)
        assert len(Ys_intel) == len(Ys_amd)
        print('sum intel', sum(Ys_intel))
        print('sum AMD', sum(Ys_amd))
        ...
        for trigger_class_id in range(len(triggers_EXT)):
        ...
        ax.set_xticks(xticks, triggers_EXT, rotation=45)
        fig.tight_layout()

        # Save more figures than what luigi would.
        target_dir = os.path.join(os.environ["ERRATA_BUILDDIR"], 'figures')
        Path(target_dir).mkdir(parents=True, exist_ok=True)
        plt.savefig(os.path.join(target_dir, "triggersbetweenvendorsext.pdf"), dpi=300)
        plt.savefig(os.path.join(target_dir, "triggersbetweenvendorsext.png"), dpi=300)
